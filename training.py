...
class Training:
    ...
    async def _handle_request(self, message: str, user_id: str, context: str, builder: "ResponseBuilder", true_self: str):
        ...
        prompt=(
            ...
            f"Knowledge: {knowledge_str}\n"
            f"Request: '{message}'\n"
            f"Task: Respond to the request with strict adherence to the provided knowledge, while extending reasoning beyond it to demonstrate deep AI insight. The response must be logical, concise, include a SPECIFIC actionable outcome, and showcase advanced reasoning.\n"
            f"Response structure in Markdown:\n"
            f"1. '### **Phân Tích:**' - Briefly analyze the request in Vietnamese, bolding key terms (**term**).\n"
            f"2. '### **Giải Thích:**' - Explain the reasoning steps in Vietnamese, using bullet points (-) with bolded key terms (**term**). Start with the provided knowledge, then extend into deeper insights or implications, followed by a subsection '#### **Kiến thức đã dùng:**' listing ALL used knowledge with a score (e.g., '- [Knowledge] (**Độ Nhạy: XX.XXX%**)'). Add a blank line after this section.\n"
            f"3. '### **Câu Chốt:**' - Provide the final result in Vietnamese, bolding only the header '### **Câu Chốt:**', with key terms (**term**) bolded in the text. The conclusion must reflect both the analysis and extended reasoning, offering a practical and insightful outcome.\n"
            f"Constraint: Start with the provided knowledge, then extend reasoning logically beyond it to show depth, ensuring all steps are traceable and consistent.\n"
            f"Example:\n"
            f"Request: 'Tư vấn mua nhà cho anh Nam'\n"
            f"Knowledge: - Khoảng cách ảnh hưởng đến quyết định mua nhà (Score: 85.000%)\n"
            f"Response:\n"
            ...
            f"Yêu cầu là tư vấn mua **nhà** cho **anh Nam**.  \n"
            f"### **Giải Thích:**  \n"
            f"- Dựa vào kiến thức, **khoảng cách** là yếu tố chính ảnh hưởng đến quyết định mua nhà.  \n"
            f"- Suy luận sâu hơn: **Khoảng cách ngắn** không chỉ tiết kiệm thời gian mà còn giảm chi phí đi lại, ảnh hưởng lâu dài đến chất lượng sống.  \n"
            f"- Nếu anh Nam làm việc ở thành phố, nhà gần trung tâm có thể tăng giá trị tài sản theo thời gian, dù giá ban đầu cao hơn.  \n"
            f"#### **Kiến thức đã dùng:**  \n"
            f"- Khoảng cách ảnh hưởng đến quyết định mua nhà (**Độ Nhạy: 85.000%**).  \n"
            f"\n"
            f"### **Câu Chốt:**  \n"
            f"Anh Nam nên chọn **nhà gần chỗ làm** để tiết kiệm thời gian và tiền bạc, ưu tiên khu vực trung tâm nếu có thể—vừa tiện, vừa đầu tư lâu dài."
        )
        try:
            response = await asyncio.to_thread(LLM.invoke, prompt)
            cleaned_response = response.content.strip()
