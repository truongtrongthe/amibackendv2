...
in the main application after applying the event loop isolation fixes.
"""
    ...
    try:
        ...
        for line in response.iter_lines():
            ...
            try:
                ...
                # Print truncated data for debugging
                data_str = json.dumps(data)
                if len(data_str) > 100:
                    data_str = data_str[:100] + "..."
                print(f"Request {index}: {prefix}{data_str}")
        ...
        print(f"Request {index}: Analysis events: {analysis_events}, Message chunks: {message_chunks}")
...
def main():
    ...
    parser.add_argument("--url", default="http://localhost:5001/havefun", 
                      help="URL of the /havefun endpoint")
    parser.add_argument("--requests", type=int, default=3, 
                      help="Number of concurrent requests to make")
    parser.add_argument("--thread-id", default=None,
                      help="Thread ID to use (will be generated if not provided)")
    parser.add_argument("--message", default="Testing concurrent requests", 
    ...
    for i in range(args.requests):
        ...
        thread = threading.Thread(
            target=make_request,
            args=(args.url, thread_id, args.message, i+1, delay)
        )
