...
CATEGORY_KEYWORDS = {
    ...
    "Products and Services": ["HITO", "sản phẩm", "có", "chứa", "glucosamine", "iphone", "galaxy", "tesla", "coffee"],
...
# Known products (grows dynamically)
KNOWN_PRODUCTS = {"HITO_1"}  # Start with known ones, add confirmed TEMPs later
...
def extract_knowledge(state, user_id=None):
    ...
    - Dynamic Product: Scan for product names—tạo temp `product_id` nếu mới.
    - Ingredient Linking: Tie ingredients (e.g., "Glucosamine") to products in context.
    ...
    if primary_topic == "Products and Services":
        for piece in pieces:
            text_words = piece["text"].split()
            raw_words = piece["raw_input"].split()
            product_candidates = []
            # Check KNOWN_PRODUCTS first
            for known in KNOWN_PRODUCTS:
                if known in piece["text"]:
                    product_candidates.append(known)
            # Multi-word phrases up to 3 words from text, stop at non-product words
            for i in range(len(text_words)):
                for j in range(i + 1, min(i + 3, len(text_words) + 1)):
                    phrase = " ".join(text_words[i:j])
                    next_word = text_words[j] if j < len(text_words) else ""
                    if (phrase[0].isupper() and 
                        phrase.lower() not in ["glucosamine", "bột"] and  # Filter ingredients
                        (not next_word or next_word[0].isupper() or next_word.lower() in ["và", "là"])):  # Allow connectors
                        product_candidates.append(phrase)
            # Sort by length (longest first)
            product_candidates.sort(key=len, reverse=True)
            
            if product_candidates:
                piece["product_id"] = product_candidates[0]  # Take longest valid phrase from text
                if piece["product_id"].startswith("TEMP_") and "confirmed" not in state.get("temp_products", {}):
                    state["prompt_str"] = f"Ami hiểu '{piece['text']}' là sản phẩm mới—đúng không?"
            else:
                piece["product_id"] = f"TEMP_{uuid.uuid4().hex[:6]}"
                state["prompt_str"] = f"Ami hiểu '{piece['text']}' là sản phẩm mới—đúng không?"
                state.setdefault("temp_products", {})[piece["product_id"]] = piece["text"]
            
            # Ingredient linking
            ingredients = [w for w in text_words if "glucosamine" in w.lower() or "bột" in w.lower()]
    ...
    pending_node["primary_topic"] = primary_topic
...
def confirm_knowledge(state, user_id):
    ...
    if response == "yes":
        ...
        # Confirm temp products
        temp_products = state.get("temp_products", {})
        for piece in node["pieces"]:
            if piece["product_id"].startswith("TEMP_") and piece["product_id"] in temp_products:
                KNOWN_PRODUCTS.add(piece["product_id"])
                logger.info(f"Confirmed temp product: {piece['product_id']} as {temp_products[piece['product_id']]}")
        
        store_in_pinecone(node, user_id)
        time.sleep(2)  # Delay to ensure Pinecone indexes
        state["pending_node"] = {"pieces": [], "primary_topic": node["primary_topic"]}
        state.pop("temp_products", None)  # Clear temp tracking after confirmation
    elif response == "no":
...
def store_in_pinecone(node, user_id):
    ...
    metadata = {
        ...
        "linked_nodes": json.dumps([p.get("linked_entities", {}).get("product_id", "") for p in node["pieces"] if "linked_entities" in p], ensure_ascii=False),
...
def recall_knowledge(message, user_id=None):
    ...
    logger.debug(f"Query embedding: {query_embedding[:5]}... (truncated)")
...
def test_ami():
    ...
    state["messages"].append(HumanMessage("Bột xương cá tuyết"))  # Fixed input
    pending_node = extract_knowledge(state)
    confirm_knowledge(state, "user123")  # Assume "yes"
    
    # Test 3: Recall with known product
    ...
    # Test 5: New product
    state["messages"].append(HumanMessage("Apple iPhone là đỉnh cao công nghệ"))
    pending_node = extract_knowledge(state)
    confirm_knowledge(state, "user123")  # Assume "yes"
    result = recall_knowledge("Apple iPhone có gì hay?", "user123")
    logger.info(f"Recall result (new product): {json.dumps(result, ensure_ascii=False)}")
