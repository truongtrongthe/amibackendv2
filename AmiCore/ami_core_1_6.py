# ami_core_v1.6_with_dynamic_brain
...
class AmiCore:
    ...
    def detect_intent(self, msg):
        ...
        - "teaching": Statements with intent to inform (e.g., "HITO is a…", "You need to know…").
        ...
        Prioritize content—short facts default to "casual".""").content.strip()
    ...
    def extract_entry(self, msg, intent):
        ...
        - Use 'rule' and 'action' for explicit needs (e.g., 'need to know'); 'action' for instructions with a method (e.g., 'ask them gently'); 'knowledge' for product facts unless it’s a story (e.g., 'works').
        - For rules, extract dependencies as simple fields (e.g., 'age', not 'customer age') from text like 'need to know customer age, gender, height'.
        - For stories, distill 'lesson' and 'tip'.
    ...
    def process(self, state: State, is_first: bool):
        ...
        if intent == '"teaching"':
            ...
            for e in extracted:
                if not any(b["summary"] == e["summary"] for b in self.brain):
                    self.brain.append(e)
            ...
            state["last_teaching_product"] = last_teaching_product
            return {"prompt_str": f"{pickup} {reply} What else you got?".strip(), "brain": self.brain, "messages": state["messages"], "last_teaching_product": last_teaching_product}
        elif intent == '"casual"':
            ...
            state["last_teaching_product"] = last_teaching_product
            return {"prompt_str": reply.strip(), "brain": self.brain, "messages": state["messages"], "last_teaching_product": last_teaching_product}
        else:
...
def convo_stream(user_input=None, thread_id=f"test_thread_{int(time.time())}"):  # Unique thread_id per run
    checkpoint = checkpointer.get({"configurable": {"thread_id": thread_id}}) or {}
    state = {
        "messages": checkpoint.get("channel_values", {}).get("messages", []) + ([HumanMessage(content=user_input)] if user_input else []),
    ...
    print(f"Debug: Starting convo_stream with last_teaching_product={state['last_teaching_product']}")
    ...
    print(f"Debug: Ending convo_stream with last_teaching_product={state['last_teaching_product']}")

# Test
if __name__ == "__main__":
    # Clear Pinecone namespace once at the start of the run
    #PINECONE_INDEX.delete(delete_all=True, namespace="tfl")
    print("\nAmi starts:")
    for chunk in convo_stream():
        print(chunk)
    test_inputs = [
        "You need to know customer age, gender, height coz it affects height growth. So during customer convo you have to ask them gently.",
        "HITO is a calcium supplement.",
        "Pitch me!",
        "I’m 25.",
        "Pitch me!",
        "Male.",
    ...
    for input in test_inputs:
        print(f"\nYou: {input}")
        for chunk in convo_stream(input):
            print(chunk)