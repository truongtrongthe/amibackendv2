# ami_core_v1.7_with_staged_rules
# Built by: The Fusion Lab with xAI's Grok 3
# Date: March 13, 2025
# Purpose: Ami's Training + Staged Selling with Multiple Rules
...
class AmiCore:
    ...
    def detect_intent(self, msg):
        ...
        - "casual": Short, conversational replies or facts (e.g., "I’m 25", "Male", "Full", "123 Main St"), especially after prompts like 'pay' or 'address'.
        - "selling": Pitch requests (e.g., "Pitch me!").
        - "buying": Agreement to buy (e.g., "Yes", "I’ll take it").
        Prioritize content—short facts default to "casual" unless clearly instructional."""
    ...
    def extract_entry(self, msg, intent):
        ...
        for entry in entries:
            ...
            if entry["type"] == "action" and "gently" in msg.lower() and "method" not in entry:
                entry["method"] = "gently"
        print(f"Debug: Extracted entries for '{msg}': {entries}")
        return entries

    def store_customer_data(self, msg, intent, last_ami_msg):  # Use last_ami_msg
        if intent != '"casual"':
            return []
        prompt = f"""Given prior message '{last_ami_msg}', if '{msg}' contains customer info, return JSON with:
        - 'summary': fact (e.g., 'Customer is 25', 'Payment is full')
        - 'field': data type (e.g., 'age', 'gender', 'height', 'payment_type', 'shipping_address')
        - 'value': value (e.g., '25', 'male', '5’6', 'full', '123 Main St')
        Recognize short inputs: 'Male' as gender, '5’6' as height, 'full' or 'partially' as payment_type (especially after payment questions), addresses as shipping_address. Else return empty JSON {{}}. Return ONLY the JSON, no extra text. Wrap in ```json``` and ```."""
        response = llm_non_streaming.invoke(prompt).content.strip()
        print(f"Debug: Customer data response for '{msg}' with prior '{last_ami_msg}': {response}")
        start = response.find("```json") + 7
        end = response.rfind("```")
        if start <= 6 or end <= start:
            return []
    ...
    def process(self, state: State, is_first: bool):
        ...
        if intent == '"teaching"':
            ...
            customer_data = self.store_customer_data(msg, intent, last_ami_msg)  # Pass last_ami_msg
            reply = f"Cool, noted {customer_data[0]['summary']}!" if customer_data else llm.invoke(f"Respond to '{msg}'—sharp, charming.").content
            if customer_data and last_teaching_product and sales_stage != "done":  # Add nudge
                reply += " Say 'Pitch me!' to continue"
            ...
            if missing_deps and product:
                ...
                reply = f"{pickup} {product.upper()}’s great—{question}"
                ...
                if sales_stage == "profiling":
                    sales_stage = "pitched"
                self.brain = [b for b in self.brain if b.get("type") != "conversation_goal"]
            elif not missing_deps and sales_stage == "payment" and any(cd["field"] == "payment_type" for cd in customer_data):  # Check customer_data
                reply = "Payment confirmed! What’s your shipping address?"
                sales_stage = "shipping"
                self.brain = [b for b in self.brain if b.get("type") != "conversation_goal"]
            elif not missing_deps and sales_stage == "shipping" and any(cd["field"] == "shipping_address" for cd in customer_data):  # Check customer_data
                reply = f"{product.upper()}’s on its way! Anything else?"
                sales_stage = "done"
                self.brain = [b for b in self.brain if b.get("type") != "conversation_goal"]
            else:
