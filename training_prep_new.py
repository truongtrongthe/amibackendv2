...
async def save_document_insights(document_insight: str = "", user_id: str = "", mode: str = "default", bank: str = "") -> bool:
    ...
    Uses a hierarchical structure with multiple specialized vectors per cluster.
    ...
    try:
        ...
        logger.info(f"Processing {len(clusters)} clusters from document insights using hierarchical structure")
        
        # Create tasks for saving each cluster with multiple vector types
        processing_tasks = []
        
        # First, save document-level summary vector
        doc_summary_text = f"Document Summary: {summary}\nConnections: {connections}"
        doc_chunk_id = f"doc_{doc_id}_summary"
        processing_tasks.append(
            save_training_with_chunk(
                input=doc_summary_text,
                user_id=user_id,
                mode=mode,
                doc_id=doc_id,
                ...
                is_raw=True,
                custom_metadata={
                    "vector_type": "document_summary",
                    "standalone": True,
                    "cluster_count": len(clusters),
                    "related_clusters": [f"cluster_{doc_id}_{cluster.get('id', 'unknown')}" for cluster in clusters]
                }
            )
        )
        
        # Now save each cluster with specialized vectors
        for cluster in clusters:
            cluster_id = cluster.get("id", "unknown")
            cluster_title = cluster.get("title", "Untitled Cluster")
            cluster_description = cluster.get("description", "")
            cluster_sentences = cluster.get("sentences", [])
            cluster_takeaways = cluster.get("takeaways", "")
            
            # Generate a base chunk ID for this cluster
            base_chunk_id = f"cluster_{doc_id}_{cluster_id}"
            
            # Get related cluster IDs (for metadata)
            related_clusters = [f"cluster_{doc_id}_{c.get('id', 'unknown')}" for c in clusters if c.get('id') != cluster_id]
            
            # 1. Save concept/title vector
            concept_text = f"Title: {cluster_title}\nConcepts: {cluster_title}"
            processing_tasks.append(
                save_training_with_chunk(
                    input=concept_text,
                    user_id=user_id,
                    mode=mode,
                    doc_id=doc_id,
                    chunk_id=f"{base_chunk_id}_concept",
                    bank_name=bank,
                    is_raw=True,
                    custom_metadata={
                        ...
                        "content_metadata": {
                            "primary_concepts": [cluster_title],
                            "semantic_tags": [cluster_title.split()[-1] if len(cluster_title.split()) > 1 else cluster_title]
                        },
                        "categories_special": "concept"
                    }
                )
            )
            
            # 2. Save key sentences vector (representative content)
            # Join a subset of the most important sentences
            key_sentences = "\n".join(cluster_sentences[:5] if len(cluster_sentences) > 5 else cluster_sentences)
            processing_tasks.append(
                save_training_with_chunk(
                    input=key_sentences,
                    user_id=user_id,
                    mode=mode,
                    doc_id=doc_id,
                    chunk_id=f"{base_chunk_id}_sentences",
                    bank_name=bank,
                    is_raw=True,
                    custom_metadata={
                        "cluster_id": cluster_id,
                        "vector_type": "sentences",
                        "standalone": True,
                        "origin": {
                            "doc_id": doc_id,
                            "extraction_date": datetime.now().isoformat()
                        ...
                        "categories_special": "description"
                    }
                )
            )
            
            # 3. Save descriptive content vector (full explanation)
            description_text = f"{cluster_description}\n\nFull Content:\n{' '.join(cluster_sentences)}"
            processing_tasks.append(
                save_training_with_chunk(
                    ...
                    mode=mode,
                    doc_id=doc_id,
                    chunk_id=f"{base_chunk_id}_description",
                    bank_name=bank,
                    is_raw=True,
                    custom_metadata={
                        "cluster_id": cluster_id,
            ...
            # 4. Save actionable insights vector (application-focused)
            insights_text = f"{cluster_takeaways}"
            processing_tasks.append(
                save_training_with_chunk(
                    input=insights_text,
                    user_id=user_id,
                    mode=mode,
                    doc_id=doc_id,
                    chunk_id=f"{base_chunk_id}_insights",
                    bank_name=bank,
                    is_raw=True,
                    custom_metadata={
                        "cluster_id": cluster_id,
                        "vector_type": "insights",
                        "standalone": True,
                        "origin": {
                            "doc_id": doc_id,
                            "extraction_date": datetime.now().isoformat()
                        ...
                        "categories_special": "actionable"
                    }
                )
            )
            
            # 5. Save connections vector (relationships with other clusters)
            connections_text = f"Title: {cluster_title}\nConnections with other topics: {connections}"
            processing_tasks.append(
                save_training_with_chunk(
                    input=connections_text,
                    user_id=user_id,
                    mode=mode,
                    doc_id=doc_id,
                    chunk_id=f"{base_chunk_id}_connections",
                    bank_name=bank,
                    is_raw=True,
                    custom_metadata={
                        "cluster_id": cluster_id,
                        "vector_type": "connections",
                        "standalone": False,  # Connections depend on other clusters
                        "origin": {
                            "doc_id": doc_id,
                            "extraction_date": datetime.now().isoformat()
                        },
                        ...
                        "categories_special": "relationship"
                    }
                )
            )
        
        # Process all save tasks
        logger.info(f"Executing {len(processing_tasks)} save tasks for hierarchical structure")
        ...
        logger.info(f"Successfully saved {success_count}/{len(processing_tasks)} vectors using hierarchical structure")
        return True
