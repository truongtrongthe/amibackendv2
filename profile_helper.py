...
async def llm_based_profiling(conversation_context: str, profiling_knowledge: List[Dict] = None) -> Dict:
    ...
    try:
        ...
        # Add dynamic profiling instructions
        ...
        Only return valid JSON without any explanation.
        """
        
        try:
            # Use the LangChain ChatOpenAI interface properly
            messages = [
                SystemMessage(content="You are an expert psychologist specializing in Vietnamese sexual health customer profiling."),
                HumanMessage(content=prompt)
            ]
            
            # Call the model (ainvoke is the async version of invoke)
            response = await LLM.ainvoke(messages)
            
            # Parse JSON from content
            try:
                profile_result = json.loads(response.content)
                return profile_result
            except json.JSONDecodeError:
                logger.error(f"Error parsing JSON from LLM response: {response.content[:100]}")
                return {}
