...
async def infer_categories(input: str, context: str = "") -> Dict:
    ...
    Rules:
    - Name: If input sets AI name (e.g., "You're <name>", "em hãy nhớ em tên là <name>"):
      - "primary": "name", "special": "character".
      - Label: "Tên" (Name).
    - Instincts: If input defines AI traits with directives (e.g., 'be curious', 'always be truthful', 'should be curious'):
      - "special": "character".
      - "primary": Traits (e.g., "curiosity, truthfulness").
      - Label: 'Tính Cách' (character) per trait.
    - Knowledge: If input is a fact/method unrelated to AI personality:
      - "primary": Topic (e.g., "curiosity", "sales skills").
      - "special": "description" if explanatory (e.g., "means", "is", "là"), "" if procedural (e.g., "cần").
    - Unclear: 'needs_clarification': true.
...
async def query_knowledge_fn(user_id: str, query: str, top_k: int = 3) -> List[Dict]:
...
async def generate_response(user_id: str, query: str) -> str:
    ...
    query_results = await query_knowledge_fn(user_id, query)
    if query_results:
