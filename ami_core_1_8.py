    ...
    def process(self, state: State, is_first: bool):
        ...
        if intent == '"teaching"':
                ...
                if goals:
                    ...
                    if not goal["remaining_deps"]:
                        if goal["stage"] == "profiling":
                            ...
                            sales_stage = "done"
                            reply = f"{last_teaching_product.upper()}’s on its way! Anything else? Say 'Pitch me!' for more"  # Post-sale nudge
            ...
            if missing_deps and product:
                ...
                reply = f"{product.upper()}’s on its way! Anything else? Say 'Pitch me!' for more"  # Post-sale nudge
                sales_stage = "done"
                self.brain = [b for b in self.brain if b.get("type") != "conversation_goal"]
            else:
