    ...
    def do(self, state=None, is_first=False, confirm_callback=None):
        ...
        if intent == "teaching":
            ...
            if not recall["knowledge"]:
                ...
                Return a chill, sales-y response in Vietnamese—blend all knowledge, use exact key phrases (e.g., "ổn định hấp thụ xương"), 
                make sales hooks explicit (e.g., "mua cho con"). If it’s a component (e.g., "Aquamin F"), 
                tie it to "HITO Cốm". Predict objections (e.g., age, cost) and hit ‘em with a "cực chất" vibe. 
                Keep it short, actionable—drop a "nè" if it fits!"""
