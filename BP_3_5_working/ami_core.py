    ...
    async def do(self, state=None, is_first=False, confirm_callback=None, force_copilot=False, user_id=None):
        ...
        if force_copilot:
            ...
            if intent == "teaching":
                ...
                recall = await asyncio.to_thread(recall_knowledge, latest_msg, user_id)
                logger.debug(f"Recall result: {json.dumps(recall, ensure_ascii=False)}")
                if not recall or "knowledge" not in recall or not recall["knowledge"]:
                    logger.info("NO KNOWLEDGE FOUND")
                    unknown_term = " ".join(latest_msg.split()[2:]) if " " in latest_msg else latest_msg
                    response = f"Kể tôi nghe thêm về {unknown_term} của anh đi, để tôi giúp bán!"
                else:
        ...
        self.state = state
        return state
    async def doNOTOK(self, state=None, is_first=False, confirm_callback=None, force_copilot=False, user_id=None):
        state = state if state is not None else self.state
        latest_msg = state["messages"][-1].content if state["messages"] else ""
        response = ""
        user_id = user_id or state.get("user_id", "unknown")
        ...
        # Context summary for transitions
        context_msgs = state["messages"][-10:] if len(state["messages"]) >= 10 else state["messages"]
        weights = [0.5] + [0.3] * min(3, len(context_msgs)-1) + [0.2 / max(1, len(context_msgs)-4)] * (len(context_msgs)-4)
        context_summary = " ".join(m.content for m in context_msgs[:len(weights)])
        prior_intent = state.get("intent", "greeting")
        active_terms = state.get("active_terms", {})
        transition = f"Anh vừa nói {list(active_terms.keys())[0]} hả? " if active_terms else ""
        tone_base = "chill" if "bro" in state.get("last_response", "").lower() else "sharp"
        ...
        if force_copilot:
            if not latest_msg:
                ...
                state["intent"] = intent
                if intent in ["greeting", "casual"]:
                    casual_prompt = f"""You’re Ami, a chill coworker in CoPilot mode. Given:
                        - User: '{user_id.split('_')[0]}'
                        - Input: '{latest_msg}'
                        - Knowledge: {json.dumps(recall.get('knowledge', []), ensure_ascii=False)}
                        Respond in Vietnamese—short, vibey, sharp. Match tone: {tone_base}. 
                        Output MUST be a raw string."""
                    response = (await asyncio.to_thread(LLM.invoke, casual_prompt)).content.strip()
                else:
                    if not recall["knowledge"]:
                        response = f"{user_id.split('_')[0]}, chưa đủ info—bro thêm tí nha!"
                    else:
                        prompt = f"""You’re Ami, a Sales Expert in CoPilot mode. Given:
                            - User: '{user_id.split('_')[0]}'
                            - Input: '{latest_msg}'
                            ...
                            - Knowledge: {json.dumps(recall["knowledge"], ensure_ascii=False)}
                            - Terms: {json.dumps(recall["terms"], ensure_ascii=False)}
                            Return a Vietnamese instruction—vivid, persuasive, using exact phrases from knowledge/terms. 
                            Match tone: {tone_base}. Include an example in bold italics (e.g., **_'example'_**).
                            Output MUST be a raw string, no extra markdown except example."""
                        response = (await asyncio.to_thread(LLM.invoke, prompt)).content.strip()
                state["copilot_task"] = latest_msg if latest_msg else None
            state["prompt_str"] = f"Co Pilot: {transition}{response}"

        else:
            intent = await asyncio.to_thread(detect_intent, state) if latest_msg else "greeting"
            state["intent"] = intent
            ...
            if intent == "teaching":
                ...
                if confirmed_node and state["last_response"] == "yes":
                    terms = knowledge["terms"]
                    state["pending_knowledge"] = terms
                    chunk_list = [f"{term_id.split('term_')[1].rsplit('_', 1)[0]}: {chunk}" 
                                for term_id, term_data in terms.items() for chunk in term_data.get("knowledge", [])]
                    prompt = f"""You’re Ami, vibin’ for Teaching Mode 2.0. Given:
                                - User: '{user_id.split('_')[0]}'
                                - Input: '{latest_msg}'
                                - Chunks: {json.dumps(chunk_list, ensure_ascii=False)}
                                Return a Vietnamese response:
                                - Hook: 'Anh Teacher, đỉnh thật!'
                                - Analyze: 'Em thấy anh dạy [term] thì [chunk]—vibe [category] hả bro?'
                                - Demo: 'Ví dụ nhé: [example based on chunk].'
                                - Confirm: 'Chuẩn chưa anh? Hay chỉnh gì thêm?'
                                - Match tone: {tone_base}."""
                    response = (await asyncio.to_thread(LLM.invoke, prompt)).content.strip()
                    if state["sales_stage"] == "profiling":
                        response += " Giờ anh bán thử HITO Cốm với mẹo này không?"
                state["prompt_str"] = f"{transition}{response}"

            elif intent in ["question", "request"]:
                recall = await asyncio.to_thread(recall_knowledge, latest_msg, user_id)
                if not recall or "knowledge" not in recall or not recall["knowledge"]:
                    response = f"{transition}Kể tôi nghe thêm về {' '.join(latest_msg.split()[2:])} của anh đi!"
                else:
                    prompt = f"""You’re Ami, pitching smooth. Given:
                        - User: '{user_id.split('_')[0]}'
                        - Input: '{latest_msg}'
                        - Intent: '{intent}'
                        - Knowledge: {json.dumps(recall["knowledge"], ensure_ascii=False)}
                        - Terms: {json.dumps(recall["terms"], ensure_ascii=False)}
                        Return a Vietnamese pitch—blend knowledge into a tight GAIN play. Match tone: {tone_base}.
                        Output MUST be a raw string."""
                    response = (await asyncio.to_thread(LLM.invoke, prompt)).content.strip()
                    if intent == "request" and recall["mode"] == "Autopilot":
                        state["sales_stage"] = self.sales_stages[1]
                        response += " Chốt đơn luôn nhé bro?"
                state["prompt_str"] = f"{transition}{response}"

            elif intent in ["greeting", "casual"]:
                pickup = self.get_pickup_line(is_first, intent)
                casual_prompt = f"You're Ami, chilling with '{latest_msg}'. Respond in Vietnamese, vibe: '{pickup}' for {user_id.split('_')[0]}. Match tone: {tone_base}."
                response = (await asyncio.to_thread(LLM.invoke, casual_prompt)).content
                state["prompt_str"] = f"{transition}{response}"
